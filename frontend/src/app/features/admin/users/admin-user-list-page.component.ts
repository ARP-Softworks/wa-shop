...
import { AdminUser, PageResponse } from '../../../shared/models/admin.models';
...
export class AdminUserListPageComponent implements OnInit {
  ...
  readonly state = signal<UiState<PageResponse<AdminUser>>>(loadingState());
        ...
        next: (response) => {
          this.state.set(
            response.content.length === 0 && this.page() === 0
              ? { status: 'empty', message: 'No hay usuarios registrados todavía.' }
              : successState(response)
          );
        },
        error: (error) => {
          this.state.set(errorState(apiErrorMessage(error, 'No se pudieron cargar los usuarios')));
        },
      });
  }
